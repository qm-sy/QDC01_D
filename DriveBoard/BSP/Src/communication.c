...
void Uart1_Send_Statu_Init( void )
{
    ...
    sata.TX1_buf[128] = 0;
    sata.RX1_buf[128] = 0;
    sata.TX1_send_bytelength = 0;
    sata.TX1_send_cnt = 0;
    sata.RX1_rev_timeout = 0;
    sata.RX1_rev_cnt = 0;
}
...
{   
    ...
    if( TI == 1 )          //
    {
        ...
        /* 3, 依次将TX4_buf中数据送出（写S4BUF操作即为发送）    */
        if( sata.TX1_send_bytelength != 0 )
        {
            SBUF = sata.TX1_buf[sata.TX1_send_cnt++];
            sata.TX1_send_bytelength--;
        }else
        {
            sata.TX1_send_cnt = 0;
        }
    ...
    if( RI == 1 )
    {
        ...
        RI = 0;
        /* 3, 判断数据包是否接收完毕                           */
        if( !sata.RX1_rev_end_Flag )
        {
            /* 4, 数据包大于RX_buf 则从头计数                  */
            if( sata.RX1_rev_cnt > 128 )
            {
                sata.RX1_rev_cnt = 0;
            }

            /* 5, 依次将RX4_buf中数据接收（读S2BUF操作即为接收）*/
            sata.RX1_buf[sata.RX1_rev_cnt] = SBUF;
            sata.RX1_rev_cnt++;
        }
        /* 6, 重置接收完毕判断时间                              */
        sata.RX1_rev_timeout = 5;
    }
}
...
void Tim0_ISR( void ) interrupt 1   //1ms
{
    ...
    /* 1, 如果接收未超时                                             */
    if ( sata.RX1_rev_timeout != 0 )  
    {
        sata.RX1_rev_timeout--;
        /* 2, 如果接收超时                                          */
        if( sata.RX1_rev_timeout == 0 )  
        {
            if( sata.RX1_rev_cnt > 0 )  
            {   
                 /* 3, 接收完毕标志位亮起并初始化接收缓冲区         */
                sata.RX1_rev_end_Flag = 1;    
            }
        }
    } 
}
